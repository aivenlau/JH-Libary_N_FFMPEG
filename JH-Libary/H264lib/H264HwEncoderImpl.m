...
{
    __weak H264HwEncoderImpl *weakself =self;
    dispatch_sync(aQueue, ^{
        ...
        VTSessionSetProperty(weakself.EncodingSession, kVTCompressionPropertyKey_ProfileLevel, kVTProfileLevel_H264_Baseline_4_1);
        ...
        VTSessionSetProperty(weakself.EncodingSession, kVTCompressionPropertyKey_AverageBitRate, ref);
        CFRelease(ref);

        int frameInterval = 10; //关键帧间隔 越低效果越屌 帧数据越大
        CFNumberRef  frameIntervalRef = CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &frameInterval);
        VTSessionSetProperty(weakself.EncodingSession, kVTCompressionPropertyKey_MaxKeyFrameInterval,frameIntervalRef);
...
- (void) encode:(CMSampleBufferRef )sampleBuffer
{
    ...
    dispatch_sync(aQueue, ^{
        ...
        CMTime presentationTimeStamp = CMTimeMake(weakself.frameCount, 1000);
        ...
        if (statusCode != noErr)
        {
            if (weakself.EncodingSession!=nil||weakself.EncodingSession!=NULL)
            {
                VTCompressionSessionInvalidate(weakself.EncodingSession);
                CFRelease(weakself.EncodingSession);
                weakself.EncodingSession = NULL;
                return;
            }
        }
    });

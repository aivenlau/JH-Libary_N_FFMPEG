...
{
    Float64             _mSampleRate;
    AudioFormatID       _mFormatID;
    AudioFormatFlags    _mFormatFlags;
    UInt32              _mBytesPerPacket;
    UInt32              _mFramesPerPacket;
    UInt32              _mBytesPerFrame;
    UInt32              _mChannelsPerFrame;
    UInt32              _mBitsPerChannel;
}
...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    self->_mSampleRate=inAudioStreamBasicDescription.mSampleRate;
    self->_mFormatID=inAudioStreamBasicDescription.mFormatID;
    self->_mFormatFlags=inAudioStreamBasicDescription.mFormatFlags;
    self->_mBytesPerPacket=inAudioStreamBasicDescription.mBytesPerPacket;
    self->_mFramesPerPacket=inAudioStreamBasicDescription.mFramesPerPacket;
    self->_mBytesPerFrame=inAudioStreamBasicDescription.mBytesPerFrame;
    self->_mChannelsPerFrame=inAudioStreamBasicDescription.mChannelsPerFrame;
    self->_mBitsPerChannel=inAudioStreamBasicDescription.mBitsPerChannel;
    
    
    AudioStreamBasicDescription outAudioStreamBasicDescription = {0}; // 初始化输出流的结构体描述为0. 很重要。
    outAudioStreamBasicDescription.mSampleRate = 44100;//inAudioStreamBasicDescription.mSampleRate; // 音频流，在正常播放情况下的帧率。如果是压缩的格式，这个属性表示解压缩后的帧率。帧率不能为0。
    ...
    if (status != 0) {
        NSLog(@"setup converter_22: %d", (int)status);
    }
    
    UInt32 value = 0;
    UInt32 size = sizeof(value);
    status = AudioConverterGetProperty(_audioConverter, kAudioConverterPropertyMaximumOutputPacketSize, &size, &value);
    self->_mFramesPerPacket = value;
    self->_mSampleRate = 44100;
}
/**
 *  获取编解码器
 *
...
- (void) encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer completionBlock:(void (^)(NSData * encodedData, NSError* error))completionBlock {
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        size_t pcmBufferSizeA;
        char *pcmBufferA;
        
        OSStatus status = CMBlockBufferGetDataPointer(blockBuffer, 0, NULL, &pcmBufferSizeA, &pcmBufferA);
        weakself.pcmBufferSize =pcmBufferSizeA;
        weakself.pcmBuffer = (char *)(pcmBufferA);
        ...
        outAudioBufferList.mBuffers[0].mDataByteSize = (int)self.aacBufferSize;
        ...
        if (status == 0) {
            ...
            //data = fullData;
            data = [NSMutableData dataWithData:rawAAC];
